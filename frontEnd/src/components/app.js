...
//components
import LoginSignUp from './login-signup/login-signup.jsx';
...
export default class App extends React.Component {
  ...
  componentDidMount() {
    fetch( '/data', {
      ...
      .then( data => {
        console.warn( data );
        this.setState( { appData: data.app, users: data.users } );
      } )
      .catch((error) => {
  ...
  createInvoice = idx => {
    let template = JSON.parse(JSON.stringify(this.state.users[0].invoices[0]));
      ...
      return (
        <div className="app">
          <LoginSignUp
            ...
            users={app.users}
          />
          <div>{ this.state.backendData }</div>
        </div>
      );
    }
  }
